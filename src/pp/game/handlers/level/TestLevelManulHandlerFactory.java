        ...
        switch (type) {
            ...
            case TEST_LOW_HP_ONE_MONSTER:
                handler.setRunnable(new Runnable() {
                    @Override
                    public void run() {
                        GameScene.getInstance().attachChild(new Spider().getAliveSprite());
            ...
            case TEST_HIGH_HP_MANY_MONSTERS:
                ...
                final MonsterType MONSTER_TYPE = new Runner();
                handler.setRunnable(new Runnable() {
                    @Override
                    public void run() {
                        for (int i = 0; i < MONSTERS_COUNT; i++) {
                            GameScene.getInstance().attachChild(MONSTER_TYPE.getAliveSprite());
                        }
                        GameScene.getInstance().unregisterUpdateHandler(handler);
                    }
                });
